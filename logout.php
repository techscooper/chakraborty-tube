...
if ($ckadmin == '1') {
	...
	$get = mysqli_query($conn,"SELECT * FROM `login_tbl` where `u_id`='$idadmin'");
	if($row=mysqli_fetch_array($get)){
		$llg = $row['llg_in'];
	}
	mysqli_query($conn,"UPDATE `login_api_keys` SET `stat`='0' WHERE `session_id`='$sessionIDGet'");
	mysqli_query($conn,"UPDATE `login_tbl` SET `llg_out`='$llg_out', `llg_in_time`='$llg' WHERE `u_id`='$idadmin'");
}
