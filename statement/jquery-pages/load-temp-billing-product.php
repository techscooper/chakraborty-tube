...
$tmpProductSl=mysqli_real_escape_string($conn,$_REQUEST['tmpProductSl']);
$returnStat=mysqli_real_escape_string($conn,$_REQUEST['returnStat']);
if($tmpProductSl!=''){
  mysqli_query($conn,"UPDATE `billing_product_item_update` SET `stat`='$returnStat' WHERE `billing_product_item_update`.`sl`='$tmpProductSl'");
}
$tempProductCnt = $gAmount = 0;
$getTempProduct=mysqli_query($conn,"SELECT * FROM `billing_product_item_update` WHERE `stat`=1 AND `invoice_no`='$invoice_no'");
  ...
  <tbody>
    ...
    while ($rowTempProduct=mysqli_fetch_array($getTempProduct)){
      ...
      <tr>
        ...
        <td class="text-center"><a href="javascript:void(0);" onclick="getTempProduct('<?php echo $invoice_no; ?>','<?php echo $tmpProductSl;?>','0')" style="color:red;"><b>Remove</b></a></td>
      </tr>
      <?php
    }
    ?>
