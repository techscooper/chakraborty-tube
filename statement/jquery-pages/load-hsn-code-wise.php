...
if($ckadmin==1){
  ...
  if($product_id!=""){ $productID="AND `billing_product_item`.`product_id`='$product_id'"; }else{ $productID=""; }
  ...
  $getStock=mysqli_query($conn,"SELECT `billing`.`invoice_date`, `billing_product_item`.`member_id`, `billing_product_item`.`invoice_no`, `billing_product_item`.`product_id`, `billing_product_item`.`quantity`, `billing_product_item`.`product_rate`, `billing_product_item`.`taxable_amount`, `billing_product_item`.`gst_percentage`, `billing_product_item`.`gst_value`, `billing_product_item`.`net_amount` FROM `billing`, `billing_product_item` WHERE `billing`.`invoice_no`=`billing_product_item`.`invoice_no` AND `billing`.`stat`=1 $productID $ftdt ORDER BY `billing_product_item`.`sl` DESC");
